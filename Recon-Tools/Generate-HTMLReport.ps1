...
.PARAMETER FuzzingPath
    Path to the fuzzing results file (e.g., results.log).
...
$fuzzResults = if ($FuzzingPath -and (Test-Path $FuzzingPath)) {
    Get-Content $FuzzingPath | ForEach-Object { $_ | ConvertFrom-Json }
} else { @() }
...
tr.status200 { background-color: #264f26; }
tr.status403 { background-color: #664422; }
tr.status301 { background-color: #223355; }
tr.status302 { background-color: #225577; }
tr.status500 { background-color: #551111; }
tr:hover {
...
<div class="tabs">
    ...
    <button class="tab-button" onclick="showTab('fuzzing')">Fuzzing Findings</button>
</div>
...
foreach ($fp in $fingerprints) {
    $rowClass = "status$($fp.Status)"
    $html += "<tr class='$rowClass'><td class='code'>$($fp.URL)</td><td>$($fp.Status)</td><td>$($fp.Server)</td><td>$($fp.XPoweredBy)</td><td>$($fp.ContentType)</td><td>$($fp.ContentLength)</td><td><div class='code'>$($fp.SetCookie)</div></td><td>$($fp.Timestamp)</td></tr>`n"
}
$html += @"
</tbody>
</table>
...
foreach ($entry in $dnsRecords) {
    ...
    foreach ($record in $entry.Records) {
        ...
        if (-not $value) { $value = "-" }
        $html += "<tr><td class='code'>$domain</td><td>$($record.Type)</td><td>$($record.TTL)</td><td class='code'>$value</td><td>$timestamp</td></tr>`n"
    }
}
$html += @"
</tbody>
...
<tbody>
"@
foreach ($entry in $fuzzResults) {
    $url = [System.Web.HttpUtility]::HtmlEncode($entry.URL)
    $status = $entry.Status
    $length = $entry.Length
    $rowClass = "status$status"
    $html += "<tr class='$rowClass'><td class='code'><a href='$url' target='_blank'>$url</a></td><td>$status</td><td>$length</td></tr>`n"
}
$html += @"
</tbody>
</table>
